...
async def search_command(
    ...
    if lucky:
        ...
        # Get user selection (synchronous input for simplicity)
        selection_input: str = input("Selection: ")
...
async def settings_test_all(ctx: click.Context) -> None:
    ...
    for module_name in sorted(haberlea.module_list):
        ...
        if test_url:
            click.echo(f"  {module_name}: test URL available - {test_url}")
        else:
            click.echo(f"  {module_name}: no test URL configured")
...
async def sessions_clear(ctx: click.Context, module: str) -> None:
    """Clear session data for a module."""
    haberlea: Haberlea = ctx.obj["haberlea"]
    module = validate_module_name(haberlea, module)

    click.echo(f"To clear session for {module}:")
    click.echo("  Delete the corresponding entry in config/loginstorage.json")
    click.echo("  The session will be recreated on next login.")
