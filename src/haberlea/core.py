...
class Haberlea:
    ...
    def __init__(
        ...
        self._settings_loader = settings_loader or JsonSettingsLoader()
    ...
    def _check_duplicate_constant(
        ...
        if is_current_private and not is_existing_private:
            ...
            pair = tuple[str, str](sorted([module, existing_module]))
            duplicates.add(pair)
...
async def haberlea_core_download(
    ...
    async def on_job_complete(job: DownloadJob) -> None:
        ...
        if extension_tasks is not None and job.download_path:
            extension_tasks.create_task(
                _run_extensions(haberlea_session, job.download_path)
            )
...
async def _run_extensions(session: Haberlea, download_path: str) -> None:
    """Runs all extensions for a completed album/playlist in background.

    Extensions are executed in priority order (lower numbers run first).

    Args:
        session: The Haberlea session.
        download_path: Path to the completed album/playlist directory.
    """
    for ext in session.extensions:
        try:
            print(f"\n=== Running Extension {ext.name} (priority: {ext.priority}) ===")
            await ext.instance.call_extension(download_path)
            print(f"=== Extension {ext.name} Completed ===\n")
        except Exception as e:
            logger.exception(
                "Extension %s failed for %s: %s", ext.name, download_path, e
            )


async def _run_extensions_finalize(session: Haberlea) -> None:
    """Runs finalize method for all extensions.
    ...
    for ext in session.extensions:
        try:
            # Only run finalize if the extension implements it (base class has default)
            await ext.instance.finalize()
        except Exception as e:
            logger.exception("Extension %s finalize failed: %s", ext.name, e)
...
async def _cleanup_modules(session: Haberlea) -> None:
    ...
    for module_name, module_instance in session.loaded_modules.items():
        try:
            ...
            logger.debug("Error closing module %s: %s", module_name, e)
