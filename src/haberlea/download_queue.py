...
class TrackTask(msgspec.Struct):
    ...
    Attributes:
        ...
        cover_temp_location: Pre-downloaded cover path.
        track_data: Pre-fetched track data from album/playlist/artist.
        album_info: Album information if part of an album.
        playlist_info: Playlist information if part of a playlist.
        m3u_playlist: M3U playlist path if applicable.
...
class DownloadQueue:
    ...
    def _generate_job_id(self, original_url: str) -> str:
        ...
        """
        # Use timestamp + url hash for uniqueness
        url_hash = hashlib.sha256(original_url.encode()).hexdigest()[:12]
        return f"{int(time.time() * 1000)}_{url_hash}"
    ...
    async def add_track(self, job_id: str, task: TrackTask) -> None:
        ...
        # Create progress task for UI
        await create_task(
            task_id=task.track_id,
            service=task.module_name,
            album=task.album_info.name if task.album_info else "",
