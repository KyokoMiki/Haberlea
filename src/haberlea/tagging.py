...
class BaseTagger(ABC):
    ...
    @abstractmethod
    def _embed_cover(self, data: bytes, max_size: int) -> None:
    ...
    def _handle_cover(self) -> None:
        ...
        if len(data) < max_size:
            self._embed_cover(data, max_size)
        else:
            print(
...
class FLACTagger(BaseTagger):
    ...
    def _create_tagger(self) -> FLAC:
        ...
        self._clean_dash_tags(tagger)
        return tagger

    def _clean_dash_tags(self, tagger: FLAC) -> None:
        """Removes useless MPEG-DASH tags from FLAC files."""
        if not isinstance(tagger.tags, VCFLACDict):
            return
        for tag in ("major_brand", "minor_version", "compatible_brands", "encoder"):
            if tag in tagger.tags:
                del tagger.tags[tag]
...
class OggBaseTagger(BaseTagger, ABC):
    ...
    def _clean_dash_tags(self, tagger: OggVorbis | OggOpus) -> None:
        """Removes useless MPEG-DASH tags."""
...
class MP3Tagger(BaseTagger):
    ...
    def _clean_dash_tags(self) -> None:
        """Removes useless MPEG-DASH tags."""
        if not isinstance(self._easy.tags, EasyID3):
            return
        for tag in ("major_brand", "minor_version", "compatible_brands", "encoder"):
            if tag in self._easy.tags:
                del self._easy.tags[tag]
        ...
        if self._id3 is not None:
            self._id3.add(
                COMM(
                    ...
                    lang="eng",
                    desc="",
                    text=self.ctx.track_info.tags.description or "",
                )
            )
...
class M4ATagger(BaseTagger):
    ...
    def _clean_dash_tags(self, tagger: EasyMP4) -> None:
        """Removes useless MPEG-DASH tags."""
        if tagger.tags is None:
            return
        for tag in ("major_brand", "minor_version", "compatible_brands", "encoder"):
