...
def build_track_tags(
    ...
    def process_value(key: str, value: Any) -> Any:
        if zfill_enabled and key in zfill_fields and value is not None:
    ...
    tags["artist"] = sanitise_name(track_info.artists[0])
    return tags
...
class PathBuilder:
    ...
    def build_album_path(self, album_id: str, album_info: AlbumInfo) -> str:
        ...
        album_path = (
            self._base_path
            + settings.global_settings.formatting.album_format.format(**album_tags)
        )
    ...
    def build_playlist_path(self, playlist_info: PlaylistInfo) -> str:
        ...
        playlist_path = (
            self._base_path
            + settings.global_settings.formatting.playlist_format.format(
                **playlist_tags
            )
        )
    ...
    def build_track_path(
        ...
        if (
            ...
            track_location_name = (
                self._base_path
                + settings.global_settings.formatting.single_full_path_format.format(
                    **track_tags
                )
            )
        elif (
            ...
            track_location_name = (
                location
                + settings.global_settings.formatting.track_filename_format.format(
                    **track_tags
                )
            )
        ...
        os.makedirs(
            track_location_name[: track_location_name.rfind("/")], exist_ok=True
        )
