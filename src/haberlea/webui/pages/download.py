...
class DownloadPage:
    ...
    def render(self) -> None:
        ...
        with ui.column().classes("w-full max-w-4xl mx-auto p-4 gap-4"):
            ui.label("音乐下载").classes("text-2xl font-bold")
    ...
    def _check_url_parameter(self) -> None:
        ...
        if url_param and self.url_input:
            ...
            ui.notify("已添加下载链接，点击开始下载", type="info")

    def _render_url_input(self) -> None:
        """Render URL input section."""
        with ui.card().classes("w-full"):
            ui.label("输入下载链接").classes("text-lg font-semibold mb-2")
            with ui.row().classes("w-full gap-2 items-end"):
                self.url_input = (
                    ui.textarea(label="URL（每行一个链接）", placeholder="https://...")
                    .classes("flex-grow")
                    .props("rows=3")
                )
                ui.button(
                    "开始下载", icon="download", on_click=self._start_download
                ).props("color=primary")

    def _render_quick_options(self) -> None:
        """Render quick options section."""
        with ui.card().classes("w-full"):
            ui.label("快速选项").classes("text-lg font-semibold mb-2")
            with ui.row().classes("gap-4 flex-wrap"):
                ui.select(
                    label="下载质量",
                    options=["minimum", "low", "medium", "high", "lossless", "hifi"],
                    value=settings.global_settings.general.download_quality,
                ).classes("w-40").bind_value(
                    settings.global_settings.general, "download_quality"
                )
                ui.checkbox(
                    "忽略已存在文件",
                    value=settings.global_settings.advanced.ignore_existing_files,
                ).bind_value(settings.global_settings.advanced, "ignore_existing_files")

    def _render_queue_card(self) -> None:
        """Render download queue card."""
        with ui.card().classes("w-full"):
            with ui.row().classes("w-full justify-between items-center mb-2"):
                ui.label("下载队列").classes("text-lg font-semibold")
                ui.button(
                    "清空", icon="delete_sweep", on_click=self._clear_queue
                ).props("flat dense")
            self._render_queue()
    ...
    def _render_log_card(self) -> None:
        """Render log card."""
        with ui.card().classes("w-full"):
            ui.label("下载日志").classes("text-lg font-semibold mb-2")
    ...
    def _render_queue(self) -> None:
        ...
        if not self._jobs:
            ui.label("队列为空").classes("text-gray-500 py-4")
            return
        ...
        with ui.card().classes(f"w-full {card_class} mb-2") as card:
            ...
            if track_ids:
                expansion = ui.expansion(
                    f"曲目列表 ({len(track_ids)})", value=expanded
                ).classes("w-full")
                ...
                with expansion:
                    ...
                    if len(track_ids) > 50:
                        ui.label(f"... 还有 {len(track_ids) - 50} 首").classes(
    ...
    async def _start_download(self) -> None:
        ...
        if self._is_downloading:
            ui.notify("下载正在进行中", type="warning")
            return

        if not self.url_input or not self.url_input.value:
            ui.notify("请输入下载链接", type="warning")
            return

        urls = [
            url.strip()
            for url in self.url_input.value.strip().split("\n")
            if url.strip() and url.startswith("http")
        ]

        if not urls:
            ui.notify("请输入有效的下载链接", type="warning")
            return
    ...
    async def download_urls(self, urls: list[str]) -> None:
        ...
        try:
            ...
            for service, media_list in media_to_download.items():
                self._log(f"服务 {service}: {len(media_list)} 个媒体")
            ...
            self._log(f"完成: {len(completed)} 成功, {len(failed)} 失败")
    ...
    async def _run_download_with_queue(
        ...
        """Runs download and registers the queue for UI updates.

        This is a modified version of haberlea_core_download that exposes
        the download queue for UI progress tracking.
        ...
        # Phase 1: Queue all media items
        for module_name, items in media_to_download.items():
            module = await haberlea_session.load_module(module_name)
            for media in items:
                await self._queue_media_item(
                    downloader, module_name, module, media, separate_download_module
                )

        # Register jobs for UI display
        self._register_jobs_from_queue(queue)
        self._log(f"已添加 {queue.job_count} 个任务, {queue.track_count} 首曲目")
        ...
        match media_type:
            case DownloadTypeEnum.album:
                await downloader.queue_album(
                    media_id, module_name, module, original_url=original_url
                )
